...
export const NavBar = () => {
  ...
  return (
    <div className='flex md:flex-row flex-col-reverse justify-between mb-[35px] gap-6'>
      <div className='lg:flex-1 flex flex-row max-w-[458px] py-2 pl-4 pr-2 h-[52px] bg-[#1c1c24] rounded-[10px]' >
        <input type="text" placeholder='Search for campaigns' className='flex-1 bg-transparent text-[#fff] outline-none placeholder:text-[#4b5264]' />
            ...
            {navlinks.map((link) => (
              ...
              >
                <img src={link.imgUrl} alt={link.name}
                  className={`w-[24px] h-[24px] ${isActive === link.name ? 'grayscale-0' : 'grayscale'}`}
                />
                <p className={`transform-gpu ml-[20px] font-epilogue font-semibold text-[14px] ${isActive === link.name ? 'text-[#1dc071]' : 'text-[#808191]'}`}>{link.name}</p>
              </li>
            ))}
          </ul>
